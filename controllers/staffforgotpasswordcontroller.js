...
const staffForgetPassword = async (req, res) => {
    ...
    try {
        const oldUser = await Userstaff.findOne({ email });
        if (!oldUser) {
        ...
        const link = `http://localhost:3000/staffchange/${oldUser._id}/${token}`;
...
const staffResetPasswordBeforeSubmit = async (req, res) => {
    const { id, token } = req.params;
    try {
        const oldUser = await UserStaff.findOne({ _id: id });
        if (!oldUser) {
            return res.json({ status: "User Not Exists!!" });
        }
        const secret = JWT_SECRET + oldUser.password;
...
const staffResetPasswordAfterSubmit = async (req, res) => {
    const { id, token } = req.params;
